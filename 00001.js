...
function showurl(datajson) {
    ...
    for (var i = 0; i < linktag.length; i++) {
        ...
        if (check == false) {
            ...
            var originalHref = linktag[i].href;
            var isExternal = !extractDomain(originalHref).includes(currentDomain);
            ...
            for (var j = 0; j < links.length; j++) {
                if (!usedLinks.has(links[j])) {
                    ...
                    if (isExternal) {
                        // Untuk link eksternal, buat redirect melalui domain saat ini dulu
                        var encrypted = aesCrypto.encrypt(convertstr(originalHref), convertstr('root'));
                        linktag[i].href = window.location.origin + "/?=" + encrypted;
                    } else {
                        // Untuk link internal, langsung ke artikel
                        linktag[i].href = feedLink + setting.path + aesCrypto.encrypt(convertstr(originalHref), convertstr('root'));
                    }
...
// Tambahkan kode untuk menangani redirect dari parameter
function handleRedirect() {
    var params = new URLSearchParams(window.location.search);
    var encryptedParam = params.get('=');
    
    if (encryptedParam) {
        var decrypted = aesCrypto.decrypt(encryptedParam, convertstr('root'));
        if (decrypted) {
            // Redirect ke blog-anda.com dengan parameter yang sesuai
            var randomArticle = links[Math.floor(Math.random() * links.length)];
            window.location.href = randomArticle + setting.path + aesCrypto.encrypt(decrypted, convertstr('root'));
            return;
        }
    }
}

// Panggil fungsi handleRedirect saat halaman dimuat
window.onload = function() {
    if (typeof showurl === 'function' && typeof handleRedirect === 'function') {
        handleRedirect();
    }
};
