...
(function(t) {
  ...
  t.formatter = {
    ...
    stringify: function(cp) {
      return cp.salt.toString() + cp.ciphertext.toString();
    },
    parse: function(str) {
      var cp = CryptoJS.lib.CipherParams.create();
      cp.salt = CryptoJS.enc.Hex.parse(str.slice(0, 32));
      cp.ciphertext = CryptoJS.enc.Hex.parse(str.slice(32));
      return cp;
    }
  };
  t.encrypt = function(text, key) {
    try {
      return CryptoJS.AES.encrypt(text, key, { format: t.formatter }).toString();
    } catch (e) { return ''; }
  };
  t.decrypt = function(token, key) {
    try {
      return CryptoJS.AES.decrypt(token, key, { format: t.formatter }).toString(CryptoJS.enc.Utf8);
    } catch (e) { return ''; }
  };
})(aesCrypto);

// Tambahkan domain saat ini ke daftar pengecualian
...
// Fungsi utama untuk memproses tautan dan mengganti href
function showurl(datajson) {
  ...
  for (var i = 0; i < datajson.feed.entry.length; i++) {
    for (var s = 0; s < datajson.feed.entry[i].link.length; s++) {
      if (datajson.feed.entry[i].link[s].rel == 'alternate') {
        links.push(datajson.feed.entry[i].link[s].href);
        break;
      }
    }
  }
  ...
  for (var i = 0; i < linktag.length; i++) {
    var link = linktag[i].href;
    var isExcepted = exceptionList.some(domain => extractDomain(link).includes(domain));
    if (!isExcepted) {
      for (var j = 0; j < links.length; j++) {
        if (!usedLinks.has(links[j])) {
          var token = aesCrypto.encrypt(convertstr(link), 'root');
          linktag[i].href = "https://www.blog-dnz.com//?=" + encodeURIComponent(token);
          
          linktag[i].rel = "nofollow";
          linktag[i].target = "_blank";
          usedLinks.add(links[j]);
          break;
        }
      }
    }
