...
// Fungsi untuk mendapatkan daftar pengecualian (URL yang tidak akan diproses)
function exception() {
...
// Fungsi untuk membersihkan string (menghapus spasi di awal dan akhir)
function convertstr(str) {
    return str.replace(/^\s+/, '').replace(/\s+$/, '');
}

// Fungsi untuk menghasilkan ID unik untuk URL
function generateUniqueId() {
    return 'ID-' + Math.random().toString(36).substr(2, 9); // Menghasilkan ID unik acak
}

// Fungsi AES Crypto untuk enkripsi dan dekripsi
var aesCrypto = {};
!function(t) {
    "use strict";
    t.formatter = {
...
// Mengecek dan menggabungkan URL pengecualian
...
// Fungsi utama untuk memproses link pada halaman
function processLinks() {
    var linkTags = document.getElementsByTagName("a"); // Ambil semua elemen <a> (tautan)
    var usedLinks = new Set(); // Set untuk memeriksa link yang sudah digunakan
    var exceptionLength = exception().length;

    // Loop untuk memproses setiap link eksternal
    for (var i = 0; i < linkTags.length; i++) {
        var currentHref = linkTags[i].href;
        var check = false;

        // Cek apakah link termasuk pengecualian
        for (var j = 0; j < exceptionLength; j++) {
            if (extractDomain(currentHref).match(extractDomain(exception()[j]))) {
                check = true;
            }
        }

        // Jika link tidak ada dalam pengecualian, ganti dengan ID unik
        if (check === false) {
            var uniqueId = generateUniqueId(); // Dapatkan ID unik
            var newUrl = "https://www.blog-dnz.com/?goto=" + uniqueId;

            // Ganti link dengan URL baru
            linkTags[i].href = newUrl;
            linkTags[i].rel = "nofollow"; // Tandai link sebagai nofollow
            linkTags[i].target = "_blank"; // Set agar link terbuka di tab baru
        }
    }
}

// Fungsi untuk memproses feed dari Blogger dan mengganti link dengan ID unik
function showurl(datajson) {
    ...
    // Ambil semua URL artikel dari feed
    ...
    // Tandai link yang sudah ada di halaman
    ...
    // Acak link agar lebih dinamis
    links = links.sort(() => Math.random() - 0.5);

    // Ganti setiap link dengan feed yang baru
    for (var i = 0; i < linktag.length; i++) {
        var check = false;
        var no = 0;
        ...
        if (check == false) {
            ...
            for (var j = 0; j < links.length; j++) {
                if (!usedLinks.has(links[j])) {
                    var feedLink = links[j]; // Gunakan link feed yang belum digunakan
...
// Panggil fungsi utama untuk memproses link ketika halaman dimuat
processLinks();
