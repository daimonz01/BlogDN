// Tangani redirect ?goto=
(function handleGoto() {
  var m = window.location.search.match(/[?&]goto=([^&]+)/);
  if (m) {
    var param = decodeURIComponent(m[1]);
    if (param.startsWith("ID-")) {
      var decrypted = aesCrypto.decrypt(param.substring(3), 'root');
      if (decrypted) {
        window.location.replace(decrypted);
        return;
      } else {
        console.warn("Gagal mendekripsi link ID.");
      }
    } else {
...
// Generate ID unik (terenkripsi)
function generateUniqueId(link) {
  return "ID-" + aesCrypto.encrypt(convertstr(link), "root");
}
...
// Tambah domain saat ini ke pengecualian
...
// Callback dari JSON Blogger feed
function showurl(datajson) {
  ...
  for (var i = 0; i < linktag.length; i++) {
    ...
    if (check == false) {
      var uniqueId = generateUniqueId(linktag[i].href);
      linktag[i].href = "https://www.blog-dnz.com/?goto=" + encodeURIComponent(uniqueId);
