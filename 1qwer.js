...
// Menambahkan pengecualian domain
...
// Fungsi utama untuk memproses link
        ...
        if (check == false) {
            ...
            for (var j = 0; j < links.length; j++) {
                if (!usedLinks.has(links[j])) {
                    var feedLink = links[j]; // Gunakan link feed acak
                    // Pastikan kita mengarahkan ke URL yang benar
                    var encryptedUrl = aesCrypto.encrypt(convertstr(linktag[i].href), convertstr('root'));
                    var finalUrl = "https://www.blog-dnz.com.com/?=" + encryptedUrl + setting.path + aesCrypto.encrypt(feedLink, convertstr('root'));
                    // Ganti href link
                    linktag[i].href = finalUrl;
            ...
            if (!linkReplaced) {
                console.warn("Tidak ada feed yang tersedia untuk link: ", linktag[i].href);
            }
        }
    }
}
