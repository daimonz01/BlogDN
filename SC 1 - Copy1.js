...
(function handleGoto() {
  ...
  if (m) {
    var id = m[1];
    var encryptedUrl = uniqueIdMap[id];
    if (encryptedUrl) {
      window.location.replace(encryptedUrl);
    } else {
      console.warn("ID tidak ditemukan di map, redirect dibatalkan.");
    }
    return;
  }
})();
...
// Tambah current domain ke exception list
...
// Callback untuk JSON feed dari Blogger
...
var encryptedUrl = finalUrl; // Sudah mengandung hasil enkripsi dari aesCrypto
uniqueIdMap[uniqueId] = encryptedUrl;
linktag[i].href = "https://www.blog-dnz.com/?goto=" + uniqueId;
